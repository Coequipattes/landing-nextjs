...
- **Auto-seed des volumes** (`docker-entrypoint.sh`) : un bind mount vide (premier `docker compose up`, aucune action manuelle requise côté hôte) masque les données initiales copiées dans l'image. L'entrypoint tourne en root, copie `reviews.json`/`gallery.json`/les photos depuis `/app/.seed/` **uniquement si absents** (idempotent — ne touche jamais à des données déjà présentes/modifiées), `chown` les points de montage, puis passe la main à l'utilisateur `site` via `su-exec`. Testé : volume vide → seed + HTTP 200 ; redémarrage avec données déjà modifiées → aucun écrasement.
- Le service `cron` (compose) n'a **aucun volume monté** — il appelle uniquement `GET http://web:3000/api/admin/reviews` en HTTP ; c'est le conteneur `web` qui lit/écrit sur son propre volume. Pas de partage de volume à gérer entre les deux services.
